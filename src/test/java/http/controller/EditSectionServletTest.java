...
@Classes(cdi = true,
        value={EditSectionServlet.class},
        cdiStereotypes = CdiMock.class)
public class EditSectionServletTest extends ServletTest{
    ...
    void successfulldoPost() throws ServletException, IOException{
        ...
        verify(service, times(1)).editSection(any(), anyInt(), any(), any());
    }
    ...
    void faildoPost() throws ServletException, IOException{
        ...
        when(servletContext.getContextPath()).thenReturn("path");
        doReturn(servletContext).when(spyServlet).getServletContext();
        doThrow(ConstraintViolationException.class).when(service).editSection(any(), anyInt(), any(), any());
